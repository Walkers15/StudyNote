...
module.exports = {
	...
	'html': function (title, list, body, update) {
		...
		for(var i = 0 ; i < authors.length ; i++){
			...
			<a href="/author_update?id=${authors[i].id}" style ="color:blue;font-size:12px">수정</a> <a href="/author_delete?id=${authors[i].id}" style ="color:blue;font-size:12px">삭제</a>
			`;
		}
		list += `<p>
		...
		for(var i = 0 ; i < authors.length ; i++){
			if(id == authors[i].id){
				...
				list +=
					`<h2>${authors[i].name}</h2>
					<p>${authors[i].profile}</p>
					<a href="/author_update?id=${sanitizeHtml(authors[i].id)}" style ="color:blue;font-size:12px">수정</a> <a href="/author_delete?id=${authors[i].id}" style ="color:blue;font-size:12px">삭제</a>
					`;
			}
		}
		list += `<p>
